...
/* The engine can only generate opcodes with three constants at a time. */
#define CONST0 1
#define CONST1 7
#define CONST2 8

/* Change here the input/output for the unit tests. */
unsigned char test_io[] = {
...
int check_fun (unsigned long long prog) {
  int i;
  int size = sizeof(test_io) / 2;
  for (i=0; i<size; i++) {
    init_regs (test_io[2*i]);
    if (eval_prog (prog)!=test_io[2*i+1])
      return 0;
  }
...
int main (int argc, char **argv) {
  ...
  for(int i = 0; i < 100; i++){
  regs[0] = i;
  unsigned long long prog = ( (unsigned long long) rand() << (8 * 4) | rand() );
  eval_prog(prog);
  print_prog(prog);
